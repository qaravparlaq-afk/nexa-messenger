...
async fn websocket(State(state): State<AppState>, headers: HeaderMap, upgrade: WebSocketUpgrade) -> Result<Response, StatusCode> {
    ...
    Ok(upgrade.max_frame_size(WS_MAX_MESSAGE).max_message_size(WS_MAX_MESSAGE).on_upgrade(move |socket| websocket_session(socket, device, state.relay)))
}
...
async fn websocket_session(mut socket: WebSocket, device: [u8; 16], relay: RelayStore) {
    ...
    loop {
        ...
        match message {
            ...
            Message::Pong(_) => {}
            Message::Text(_) => { close(&mut socket).await; return; }
