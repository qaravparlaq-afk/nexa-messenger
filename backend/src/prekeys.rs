...
impl PreKeyPublication {
    ...
    pub fn validate(&self, expected_device: [u8; 16], now_ms: u64) -> Result<(), PublicationError> {
        ...
        if !verify_signed_prekey(&self.bundle) { return Err(PublicationError::InvalidSignature); }
        Ok(())
    }
}
...
pub struct PreKeyRegistry { inner: Arc<Mutex<HashMap<[u8; 16], PreKeyPublication>>>, revocations: Arc<Mutex<HashMap<[u8; 16], Revocation>>> }
...
impl PreKeyRegistry {
    pub async fn publish(&self, publication: PreKeyPublication, now_ms: u64) -> Result<(), PublicationError> {
        ...
        if let Some(current) = guard.get(&device) { if publication.generation <= current.generation { return Err(PublicationError::StaleGeneration); } }
        if self.revocations.lock().await.contains_key(&device) { return Err(PublicationError::AlreadyRevoked); }
        guard.insert(device, publication);
        Ok(())
    }
    pub async fn get(&self, device: [u8; 16]) -> Option<PreKeyPublication> {
        self.inner.lock().await.get(&device).cloned()
    }
    pub async fn is_revoked(&self, device: [u8; 16]) -> bool { self.revocations.lock().await.contains_key(&device) }
    pub async fn revoke(&self, device: [u8; 16], generation: u64, now_ms: u64) -> Result<(), PublicationError> {
        let current = self.inner.lock().await.get(&device).cloned().ok_or(PublicationError::NotFound)?;
        if current.generation != generation { return Err(PublicationError::StaleGeneration); }
        let mut guard = self.revocations.lock().await;
        if guard.contains_key(&device) { return Err(PublicationError::AlreadyRevoked); }
        guard.insert(device, Revocation { generation, revoked_at_ms: now_ms });
        Ok(())
    }
    pub async fn get_active(&self, device: [u8; 16], now_ms: u64) -> Option<PreKeyPublication> {
        if self.is_revoked(device).await { return None; }
        let publication = self.get(device).await?;
        (publication.expires_at_ms > now_ms).then_some(publication)
    }
}
...
mod tests {
    ...
    fn publication(key: &SigningKey, generation: u64) -> PreKeyPublication {
        let public=key.verifying_key().to_bytes(); let device=crate::auth::device_id_from_public_key(&public); let agreement=[3u8;32]; let spk=[4u8;32]; let mut sb=b"NEXA/SPK/v2".to_vec(); sb.extend_from_slice(&7u32.to_be_bytes()); sb.extend_from_slice(&spk); sb.extend_from_slice(&agreement); let bundle=PreKeyBundle{protocol_version:ProtocolVersion(PROTOCOL_VERSION),device_id:device,identity_signing_key:public,identity_agreement_key:agreement,signed_prekey_id:7,signed_prekey:spk,signed_prekey_signature:key.sign(&sb).to_bytes(),one_time_prekeys:vec![OneTimePrekeyPublic{key_id:9,public_key:[5;32]}]}; let now=now_ms(); let mut p=PreKeyPublication{bundle,generation,issued_at_ms:now,expires_at_ms:now+60_000,signature:[0;64]}; p.signature=key.sign(&p.signing_bytes()).to_bytes(); p
    }
    #[tokio::test] async fn publication_is_signed_and_monotonic(){let key=SigningKey::from_bytes(&[42;32]);let r=PreKeyRegistry::default();let p1=publication(&key,1);let d=p1.bundle.device_id;assert!(r.publish(p1.clone(),now_ms()).await.is_ok());assert!(matches!(r.publish(p1,now_ms()).await,Err(PublicationError::StaleGeneration)));let p2=publication(&key,2);assert!(r.publish(p2,now_ms()).await.is_ok());assert_eq!(r.get(d).await.unwrap().generation,2);}
    #[tokio::test] async fn tampering_is_rejected(){let key=SigningKey::from_bytes(&[43;32]);let mut p=publication(&key,1);p.bundle.signed_prekey[0]^=1;let r=PreKeyRegistry::default();assert!(matches!(r.publish(p,now_ms()).await,Err(PublicationError::InvalidSignature)));let mut p=publication(&key,1);p.signature[0]^=1;assert!(matches!(r.publish(p,now_ms()).await,Err(PublicationError::InvalidSignature)));}
    #[tokio::test] async fn revocation_is_not_part_of_signed_payload_and_blocks_republish(){let key=SigningKey::from_bytes(&[44;32]);let r=PreKeyRegistry::default();let p=publication(&key,5);let d=p.bundle.device_id;r.publish(p,now_ms()).await.unwrap();r.revoke(d,5,now_ms()).await.unwrap();assert!(r.is_revoked(d).await);assert!(r.get(d).await.unwrap().verify_signature());assert!(r.get_active(d,now_ms()).await.is_none());assert!(matches!(r.publish(publication(&key,6),now_ms()).await,Err(PublicationError::AlreadyRevoked)));}
}
