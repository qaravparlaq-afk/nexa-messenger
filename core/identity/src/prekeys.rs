...
mod tests {
    ...
    #[test] fn signed_prekey_retirement_is_bounded_without_deleting_live_keys() { let identity=IdentityKey::generate(); let store=SignedPrekeyStore::new(&identity,1,0); store.rotate(&identity,2,1,1_000).unwrap(); store.rotate(&identity,3,2,1_000).unwrap(); assert_eq!(store.retired_len().unwrap(),2); assert!(matches!(store.rotate(&identity,4,3,1_000),Err(SignedPrekeyStoreError::RetiredCapacity))); assert!(store.with_key(1,999, |_| ()).is_ok()); assert!(store.with_key(2,999, |_| ()).is_ok()); assert_eq!(store.current_key_id().unwrap(),3); assert_eq!(store.retired_len().unwrap(),2); assert_eq!(store.prune(1_001).unwrap(),2); assert_eq!(store.retired_len().unwrap(),0); store.rotate(&identity,4,1_002,1).unwrap(); assert_eq!(store.current_key_id().unwrap(),4); }
