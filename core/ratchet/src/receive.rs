...
pub struct ReceiveRatchet {
    state: super::RatchetState,
    skipped: BTreeMap<u64, Zeroizing<[u8; 32]>>,
}

impl ReceiveRatchet {
    pub fn from_root(root: [u8; 32]) -> Result<Self, super::RatchetError> {
        Ok(Self {
            state: super::RatchetState::from_root(root)?,
    ...
    fn key_for(&mut self, target: u64) -> Result<Zeroizing<[u8; 32]>, ReceiveError> {
        ...
        if target - expected > MAX_SKIP {
            return Err(ReceiveError::TooFarAhead);
        }
    ...
    pub fn decrypt(
        ...
        super::RatchetState::decrypt_once(&key, packet, aad)
...
mod tests {
    ...
    fn out_of_order_then_replay_is_rejected() {
        let root = [7u8; 32];
        let mut sender = super::RatchetState::from_root(root).unwrap();
        ...
        assert!(matches!(
            receiver.decrypt(0, &p0, b"aad"),
            Err(ReceiveError::Replay)
        ));
    }
    ...
    fn excessive_skip_is_rejected() {
        ...
        let mut receiver = ReceiveRatchet::from_root(root).unwrap();
        assert!(matches!(
            receiver.key_for(MAX_SKIP + 1),
            Err(ReceiveError::TooFarAhead)
        ));
    }
}
