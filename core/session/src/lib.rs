...
pub fn initiate(
    ...
    let used_id=if let Some((id,public))=one_time_prekey_public {
        ikm.extend_from_slice(own_secret.diffie_hellman(&PublicKey::from(public)).as_bytes()); Some(id)
    } else { None };
...
pub fn respond(
    ...
    let mut ikm=Vec::with_capacity(64);
    ikm.extend_from_slice(&own_signed_prekey.diffie_hellman(&peer));
    let (used_id,dh2)=if let Some((id,otp))=own_one_time_prekey {(Some(id),otp.diffie_hellman(&peer))} else {(None,[0;32])};
...
fn derive_root(ikm:&[u8],device_id:&[u8;16],ephemeral_public:&[u8;32])->Option<SessionRoot>{
    ...
    let mut info=Vec::with_capacity(50);
    info.extend_from_slice(&ProtocolVersion(PROTOCOL_VERSION).0.to_be_bytes());
    info.extend_from_slice(device_id); info.extend_from_slice(ephemeral_public);
...
mod tests {
 ...
 #[test] fn initiator_and_responder_derive_same_root(){
  let peer_identity=IdentityKey::generate(); let peer_spk=SignedPrekey::generate();
  let record=publishable_signed_prekey(&peer_identity,1,&peer_spk); let eph=InitiatorEphemeral::generate();
  ...
  let i=initiate(&IdentityKey::generate(),eph,&bundle,&record,None).unwrap();
  let r=respond(&peer_spk,None,[7;16],[8;16],eph.public_key).unwrap();
